...
st.set_page_config(page_title="Unified AI Suite", page_icon="🧠", layout="wide", initial_sidebar_state="expanded")

st.markdown("""
<style>
.stApp { background: linear-gradient(135deg, #090909, #151d35); color: white; }
body { color: white; }
.main-title{font-size:54px;text-align:center;font-weight:800;color:#66FCF1;}
.sub{text-align:center;color:#d0d0d0;font-size:20px;}
.card{background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.12);border-radius:18px;padding:24px;backdrop-filter:blur(12px);}
.metric{text-align:center;font-size:35px;color:#66FCF1;font-weight:bold;}
.small{text-align:center;color:#bfbfbf;}
.game-box,.health-box,.gluco-box{background:rgba(13,18,36,0.92);border:2px solid #66FCF1;border-radius:18px;padding:20px;}
hr{border:1px solid #333;}
section[data-testid="stSidebar"]{background:#111827;border-right:2px solid #66FCF1;}
</style>
...
# ---------------- SIDEBAR ---------------- #
st.sidebar.title("🧠 Unified AI Suite")
page = st.sidebar.radio("Navigation", ["🏠 Home", "📝 MBTI Test", "📊 MBTI Results", "🎮 Game", "🏥 Health AI", "🩺 GlucoVision", "ℹ About"])
st.sidebar.write("---")
...
# ---------------- MBTI TEST ---------------- #
    ...
    if not st.session_state.mbti_done or st.session_state.mbti_result is None:
        ...
        st.markdown(f"<div class='card'><h1 style='color:#66FCF1;text-align:center;'>{result}</h1><p style='text-align:center;color:#e0e0e0;font-size:18px;'>{mbti_desc.get(result, 'Balanced personality.')}</p></div>", unsafe_allow_html=True)
        st.write("")
...
# ---------------- GAME ---------------- #
    ...
    for obs in st.session_state.obstacles:
        ...
        if 0 <= pos < width:
            obstacle_line[pos] = "🟥"
            if pos + 1 < width: obstacle_line[pos + 1] = "🟥"
...
# ---------------- ABOUT ---------------- #
    ...
    st.title("About Unified AI Suite")
    st.write("This single Streamlit app combines your PersonaLens AI features with the GlucoVision-style glucose section.")
    st.write("Included: MBTI test, mini game, health AI, and glucose prediction.")
    st.caption("© 2026 Unified AI Suite")
